Feature: Get Recommended Wines Feature
  Scenario: CT_Recommended_1: User likes 3 of the 5 tags of a wine (AT_40)
  ...
  Scenario: CT_Recommended_2: User has no liked tags (AT_38)
    Given The database is reset
    When User 1 gets their recommended wines
    Then User 1's recommended wines should be empty

  Scenario: CT_Recommended_3: User has disliked tags (AT_40)
    Given The database is reset
  ...
  Scenario: CT_Recommended_4: 2 Users gets their separate recommended wines (AT_40)
    Given The database is reset
    And User 1 reviews tags [2013, Etna, White Blend] from wine 1 with rating 5
  ...
  Scenario: CT_Recommended_5: Recommended wines avoid what the user has reviewed (AT_40)
    Given The database is reset
    And User 1 reviews tags [2013, Etna, White Blend] from wine 1 with rating 5
  ...
  Scenario: CT_Recommended_6: Recommended wines for a non existent user (AT_38)
    Given The database is reset
    When User -1 gets their recommended wines
    Then User -1's recommended wines should be empty