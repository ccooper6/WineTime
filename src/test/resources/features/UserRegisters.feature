Feature: Registering as a user
  Scenario: CT_Register_1: User doesn't currently have an account (AT_16)
  ...
  Scenario: CT_Register_2: User has an account and tries to reregister with same username (AT_18)
  ...
  Scenario: CT_Register_3: User registers with short password (AT_17)
  ...
  Scenario: CT_Register_4: User registers with no digits in password (AT_17)
  ...
  Scenario: CT_Register_5: User registers with short password (AT_17)
  ...
  Scenario: CT_Register_6: User registers with no name (AT_17)
    Given The user with name "" doesn't currently have an account associated to the username "username" and password "12345678"
    When The user registers with the given credentials
    Then The user will not be registered due to error in credentials

  Scenario: CT_Register_7: User registers with no username (AT_17)
